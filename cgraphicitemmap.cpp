#include <QtWidgets>
#include "cgraphicitemmap.h"

CGraphicItemMap::CGraphicItemMap(QGraphicsItem *parent) : QGraphicsObject(parent)
{
    dragOver=false;
    setAcceptDrops(true);
}
...
void CGraphicItemMap::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{}
void CGraphicItemMap::dragEnterEvent(QGraphicsSceneDragDropEvent *event)
{
    Q_UNUSED(event);
    if(1)
    {
...
void CGraphicItemMap::dragLeaveEvent(QGraphicsSceneDragDropEvent *event)
{   Q_UNUSED(event);
    dragOver=false;
    update();
}
void CGraphicItemMap::dropEvent(QGraphicsSceneDragDropEvent *event)
{
    dragOver=false;
